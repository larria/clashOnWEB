...
// 阵营取色
export function sideColors(side) {
  return side === 0
...
// 径向渐变(球体感)
export function orbFill(ctx, x, y, r, base, light) {
...
// 新增卡牌:在此加一个 case(图形简陋即可,辨识优先)
export function drawUnitIcon(ctx, cardId, cx, cy, r) {
  ctx.save();
  ctx.lineJoin = 'round';
  ctx.lineCap = 'round';
  switch (cardId) {
...
// ===== 法术特效(带时间进度 t: 1→0) =====
export function drawSpellFx(ctx, cardId, x, y, radius, t) {
  ctx.save();
  ...
  switch (cardId) {
    ...
    default: { // 通用
      ...
      ctx.beginPath(); ctx.arc(x, y, radius * CELL, 0, Math.PI*2); ctx.fill();
    }
  }
  ctx.restore();
}
