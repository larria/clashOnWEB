...
**追尾推挤的方向语义**(C++ 分离算法本身没有显式建模,两参考项目均无此概念——
Unity 用 NavMesh avoidance 自动互绕,C++ 靠对称推挤+噪声自然滑开。以下规格
来自官方游戏行为的直接观察,作为本项目的补充规格):

```
同阵营两单位追尾(后单位撞上前方单位的背面):
  径向对称推挤不得把前方单位挤向侧向——
  应沿前方单位的行进方向把它"推着走"
  (官方 pig push:正后方放野猪把雪人顶着加速前进)
净效果 = 两者速度趋同:前方单位被迫加速到接近后方单位的速度,
后方单位被顶住减速——宏观上以前方单位略快于自身原速的共同速度前进。
敌对单位仍走对称径向推挤(互绕/侧向挤开)。
```

推挤量上限 = 后方单位本帧的实际移动距离(推挤是移动力传递,不能凭空产生速度)。
